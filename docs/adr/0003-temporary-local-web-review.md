...
- Entry points: `/code-eye-web` (user — walkthrough-first: it asks the agent for stops, which then opens the web review via the tool) and `review` with `surface: "web"` (agent). Default `review` surface stays TUI.
